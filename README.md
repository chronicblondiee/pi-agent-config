...
**Last updated:** 2026-05-16 — added a **Mac alternative — mlx_lm.server on M1 Pro 32 GB** section between the CachyOS LM Studio load params and the pi.dev integration block. Later same day: ported the `mlx-server` wrapper to portable bash at [`scripts/mlx-server.sh`](./scripts/mlx-server.sh) (lifted out of `pi-config/scripts/` to the repo root since it's a runtime helper, not pi-agent configuration — identical behavior to the fish version, targets stock macOS `bash 3.2`, no associative arrays, honors `MLX_SERVER_{VENV,STATE_DIR,HOST,PORT}` env overrides) and reformatted the install snippets in the Mac section from fish to bash so they work on systems without fish; later same day: aligned the `mlx-local` provider entry in `pi-config/models.json` (and the matching README example) so its `id` matches what `mlx_lm.server` actually reports for the wrapper's default model; later same day: live-tested by starting `mlx_lm.server` 0.31.3 against the Youssofal MTPLX dir and curling `/v1/models` — discovered (a) `mlx_lm.server` reports the **full absolute path** for path-based `--model` loads, not the directory basename as previously claimed, so the id is per-machine (corrected the basename claim in the README and updated `pi-config/models.json` to `/Users/brown/.lmstudio/models/Youssofal/Qwen3.6-27B-MTPLX-Optimized-Speed` and the README example to `/Users/<you>/...` as a per-user placeholder), (b) `--draft-model` and `--num-draft-tokens` **are** supported by `mlx_lm.server` 0.31.3 — the earlier "doesn't currently support `--draft-model`" claim was stale; corrected the Pi usage notes to say vanilla speculative decoding is available (pair Qwen3.5-9B as draft for Qwen3.6-27B target) and narrowed MTPLX's remaining appeal to *MTP-aware* speculative decoding specifically. Also noted that `/v1/models` lists everything in `~/.cache/huggingface/hub/` alongside the explicitly-loaded model, so the request `model` field must exactly match the id of the model you want to run. Background: LM Studio's bundled MLX runtime (`app-mlx-generate-mac14-arm64@25`, ships `mlx_vlm 0.4.5`) doesn't yet recognize Qwen3.6's MTP architecture — loads fail with `Received N parameters not in model: mtp.*`. The new section documents a `uv`-managed env running public `mlx-lm` 0.31.3 + `mlx_lm.server` as a parallel OpenAI-compatible endpoint on `localhost:8080`, with an `mlx-local` provider added to `pi-config/models.json` alongside (not replacing) the existing CachyOS `lmstudio` entry. All Mac throughput/VRAM/context numbers are intentionally left as **needs measurement** per the CLAUDE.md "don't generalize from 7900 XTX" guardrail — pending first real load on M1 Pro 32 GB. MTPLX is flagged as a future option but currently unusable (requires ≥48 GiB unified memory). Later same day: live e2e test through pi against `mlx_lm.server` surfaced HTTP 404 `Unexpected message role.` — root cause: pi defaults to the OpenAI Responses-API `developer` role for system messages, but Qwen3.6's `chat_template.jinja` only handles `system`/`user`/`assistant`/`tool`. Fixed in-config (not server-side) by adding `"supportsDeveloperRole": false` to the `compat` block of the `mlx-local` model in both `pi-config/models.json` and the README example, which flips pi's request builder to send `system`. LM Studio masks the same issue server-side, so the `lmstudio` entries don't need the flag. Also bumped Mac `contextWindow` from 16384 → 32768 (still untested for KV pressure; flagged in the Measurements pending table as the value to drop if OOM/swap appears under load). Same day: added [`pi-config/scripts/mlx-server.fish`](./pi-config/scripts/mlx-server.fish), a `start|stop|restart|status|log|list` wrapper around `mlx_lm.server` with a small hardcoded model registry (defaults to `qwen3.6-27b` → `~/.lmstudio/models/Youssofal/Qwen3.6-27B-MTPLX-Optimized-Speed`); installs via symlink into `~/.local/bin/mlx-server`, state under `~/.local/state/mlx-server/`. Also noted that LM Studio's existing MLX downloads can be reused as-is by pointing `--model` at the `~/.lmstudio/models/<pub>/<name>/` path — no duplication needed — and that the Youssofal MTPLX directory works under plain `mlx-lm` because the MTP weights live in an unindexed `mtp.safetensors` that the standard loader silently skips (so a working Qwen3.6 27B is already on disk without re-downloading).
