...
class TodoViewer {
  ...
  render(width: number): string[] {
    ...
    if (this.todos.length === 0) {
      ...
      lines.push(
        truncateToWidth(`  ${th.fg("muted", `${done}/${this.todos.length} completed`)}`, width),
      );
      lines.push("");
...
export default function (pi: ExtensionAPI): void {
  ...
  function updateStatus(ctx: ExtensionContext): void {
    ...
    const label = todos.length === 0 ? "" : `${done}/${todos.length} todos`;
    if (label) ctx.ui.setStatus("todo-tracker", label);
  }
  ...
  pi.registerTool({
    ...
    promptSnippet: "Track progress on multi-step tasks via the todo tool",
    promptGuidelines: [
      "Use the todo tool to track progress on tasks with more than 3 steps. Add items at the start of the task, then toggle them as each step completes.",
    ],
